...
class BooksInventory:
    ...
    def add_book(self, book: Book, price=0, nr_books_in_stock=0):
    ...
    def find_stock_count(self, book_id: int):
        ...
        print(str(len(self.__books.keys())) + "###################################################################################################################")
        if book_id in self.__books.keys():
    ...
    def find_book_key(self, book):
        for book_id in self.__books.keys():
            if self.__books[book_id].title == book.title:
                return book_id

    def get_book_by_id(self, id):
        if id < len(self.__books):
            return self.__books[id]
        else:
            return None

    def search_book_by_author(self, author_find: str):
        list_of_books = []
        print(author_find)
        for book_id in self.__books.keys():
            for author in self.__books[book_id].authors:
                print(author)
                if author.full_name == author_find:
                    list_of_books.append(self.__books[book_id])
                    break
        return list_of_books

    def search_book_by_release_year(self, release_year: str):
        list_of_books = []
        for book_id in self.__books.keys():
            if release_year == self.__books[book_id].release_year:
                list_of_books.append(self.__books[book_id])

        return list_of_books

    def search_book_by_publishers(self, publisher: str):
        list_of_books = []
        for book_id in self.__books.keys():
            if publisher == self.__books[book_id].publisher.name:
                list_of_books.append(self.__books[book_id])
    ...
    def search_book_by_id(self, id):
        for book_id in self.__books.keys():
            if str(id) == str(self.__books[book_id].book_id):
                return self.__books[book_id]
        return None
...
def make_review(review_text: str, user: User, book: Book, rating, timestamp: datetime = datetime.today()):
    review = Review(book, review_text, rating)
    review.set_datetime(timestamp)
    review.set_user(user)
