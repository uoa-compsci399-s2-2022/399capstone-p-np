...
def create_modal1(table_array1,header1):
    database_layout1 = [[sg.Table(table_array1, headings=header1,
        ...
        def_col_width=20,
        max_col_width=20,
        key="-majorRequirement_table-",
        row_height=20,
        enable_events=True,
        )],
    [sg.B("return",key="-RETURN-",bind_return_key=True)],
    [sg.Input(key="-INPUT3-"),sg.B("UPDATE",key="-update-")]
    ]
    ...
    while True:
        ...
        if event == '-majorRequirement_table-':
            ind = values['-majorRequirement_table-'][0]
            database = exe.list_all_majors()
            delete_item = database[ind][0]
            table_array1['-INPUT3-'].update(delete_item)

        if event == '-update-':
            create_modal3(values['-INPUT3-'])
...
def create_modal3(majorID1):
    database_layout3 = [ 
        text_row('majorName: ','-majorName-',False),
        text_row('totalPointsNeeded: ','-totalPointsNeeded-',False),
        text_row('pointsGenEd: ','-pointsGenEd-',False),
        text_row('year: ','-year-',False),
        text_row('honours: ','-honours-',False),
        text_row('level: ','-level-',False),
        text_row('pointsAboveStage1: ','-pointsAboveStage1-',False),
        text_row('pointsAboveStage2: ','-pointsAboveStage2-',False),
        text_row('miscProblems: ','-miscProblems-',False),
        [sg.B('update_majorRequirement',key='-update_majorRequirement-')],
        [sg.B("return",key="-RETURN-",bind_return_key=True)],
        ]

    table_array3 = sg.Window("update_course",database_layout3,modal=True)

    while True:
        event, values = table_array3.read()
        if event == None or event == "-RETURN-":
            break

        if event == '-update_majorRequirement-':
            exe.update_majorreq(values['-majorName-'],values['-totalPointsNeeded-']
                                ,values['-pointsGenEd-'],values['-year-']
                                ,values['-honours-'],values['-level-']
                                ,values['-pointsAboveStage1-'],values['-pointsAboveStage2-']
                                ,values['-miscProblems-'],majorID1)
...
header1 = ["majorID","majorName","totalPointsNeeded","pointGenEd","year","honours","level","pointsAboveStage1","pointsAboveStage2","miscProblems"]
subject_array = []
