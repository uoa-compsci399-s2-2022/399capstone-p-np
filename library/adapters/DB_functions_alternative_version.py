...
page = requests.get('https://www.calendar.auckland.ac.nz/en/courses/subject-index.html')
...
while i <(len(links)-3):
    list=[]
    list.append(links[i].text.rstrip())
    list.append(links[i+1].text.rstrip())
    list.append(links[i+2].text.rstrip())
    i=i+3
    Subject_index_list.append(list)
...
class searchTool:
    def __init__(self):
        ...
        sqliteConnection = sqlite3.connect("\\".join(os.getcwd().split("\\")[:os.getcwd().split("\\").index(
            "399capstone-p-np") + 1]) + "\\library\\adapters\\399courses.db")
    ...
    def return_isolated_problems_with_course(self, course_subject, course_number):
        ...
        a = self.__cursor.execute("select * from course where subject = ? and courseNumber = ?",
    ...
    def problems_with_course(self, courseName, courseNumber, timetable):
        ...
        a = self.__cursor.execute("select * from preReq where preReqSubject = ? and preReqNumber = ?",
                                  (courseName, courseNumber))
        problems_with_course.update(
            {"prereqs": [(x[2], x[3]) for x in a.fetchall() if (x[2], x[3]) not in done_courses]})

        a = self.__cursor.execute("select * from restriction where restrictionSubject = ? and restrictionNumber = ?",
                                  (courseName, courseNumber))
        problems_with_course.update({"restrictions": [(x[2], x[3]) for x in a.fetchall() if
                                                      (x[2], x[3]) in done_courses or (x[2], x[3]) in doing]})

        a = self.__cursor.execute("select * from corequisite where corequisiteSubject = ? and corequisiteNumber = ?",
                                  (courseName, courseNumber))
        problems_with_course.update({"corequisite": [(x[2], x[3]) for x in a.fetchall() if (x[2], x[3]) not in doing]})

        problems_with_course.update(
            {"other_problems": self.return_isolated_problems_with_course(courseName, courseNumber)})
    ...
    def worst_problems_with_course(self, courseName, courseNumber, timetable):
        ...
        a = self.__cursor.execute("select * from preReq where subject = ? and courseNumber = ?",
                                  (courseName, courseNumber))
        res = [(x[0], str(x[1])) for x in a.fetchall()]
        for pre in res:
            if pre not in done_courses:
                return "You need to take: " + pre[0] + " " + pre[
                    1] + " before taking " + courseName + " " + courseNumber
        ...
        a = self.__cursor.execute("select * from corequisite where subject = ? and courseNumber = ?;",
                                  (courseName, courseNumber))
        res = [(x[0], str(x[1])) for x in a.fetchall()]
        for pre in res:
            if pre not in doing:
    ...
    def problems_with_timetable(self, timetable):
        ...
        for sem in timetable:
            ...
            probs.append([[course[0] + " " + str(course[1]),
                           self.worst_problems_with_course(course[0], course[1], new_timetable),
                           self.return_isolated_problems_with_course(course[0], course[1])] for course in sem])
            print(probs)
        return probs
...
majorRequirements.year = 2020;""")
    ...
    def points_from(self, couresSubject, courseNumber):
        ...
        a = self.__cursor.execute(
            """select course.pointsValue from course WHERE course.subject = ? and course.courseNumber = ?;""",
            (couresSubject, courseNumber))
        res = [x for x in a.fetchall()]
        for x in res:
            return x[0]
        return "Does not exist"

    def required_courses_to_graduate(self, major_type, year="2020", honours="0",second_major_type="N/A"):
    ...
    def required_over_300_level_courses_to_graduate(self, major_type, year="2020", honours="0"):
        return [x for x in self.required_courses_to_graduate(major_type, year, honours) if
                x[1][0] == "4" or x[1][0] == "5" or x[1][0] == "6" or x[1][0] == "7" or x[1][0] == "8" or x[1][
                    0] == "9"]
    ...
    def reccomended_action(self, major_type, timetable, year="2020", honours="0", second_major_type="N/A"):
        print(timetable)
        print("timetable")
        done_courses = []
        for semester in timetable:
        ...
        if req_grad != []:
            for x in might_take:
                if x not in done_courses:
                    ...
                    return "You need to get " + str(might_points) + " more points from " + ", ".join(
                        [x[0] + x[1] for x in might_take]) + " in order to graduate"

        #checks required courses for second_major:
        if second_major_type != "N/A":
            req_grad = self.required_courses_to_graduate(second_major_type, "2020")
            if req_grad != []:
                for x in req_grad:
                    if x not in done_courses:
                        return "You need to take: " + x[0] + " " + x[1] + " in order to graduate"
        # Checks if they are missing points from some group for second major
            might_takea = self.might_want_to_take_points(second_major_type, timetable, "2020")
            might_take = might_takea[0]
            might_points = might_takea[1]
            if req_grad != []:
                for x in might_take:
        ...
        for x in done_courses:
            if x[0]==code:
                if x[1][0]=="3":
                    done_points += float(self.return_course_points(x[0], x[1]))
        if float(done_points)<75:
            diff=75-float(done_points)
            return "You need to take {0} more points at Stage 3 from the {1} schedule".format(diff, major_type)

        # checks major specific points for second major
        if second_major_type !="N/A":
            code = ""
            count = 0
            for y in Subject_index_list:
                if y[1] == second_major_type:
                    code = y[0]
            for x in done_courses:
                if x[0] == code:
                    if x[1][0] == "3":
            ...
            if float(done_points) < 75:
                diff=75-float(done_points)
                return "You need to take {0} more points at Stage 3 from the {1} schedule".format(diff, second_major_type)
        ...
        #Checks capstone for second_major
        if second_major_type != "N/A":
            cap = 0
            ...
            for x in done_courses:
                if (x[1][-3:] == "399"):
                    if (x[0]==code):
                        cap = 1
            if (cap == 0):
                return "You need to enroll in the Capstone Course for {}".format(second_major_type)
            ...
            if x[len(major_type)] != major_type:
                ...
                if len(dat) > 0:
                    if (faculty == dat[0]):
                        count += 1
            if (count < 3):
                return "You need to take courses in a minimum of three subject codes listed in the BSc Schedule, you are currently only taking {}".format(
                    count)

        # checks 2 orluded in the planner
        count = 0
        faculty=""
        for x in Subject_index_list:
            if major_type==x[1]:
                faculty==x[2]

        count=0
        for x in Subject_index_list:
            for y in done_courses:
                if x[0]==y[0]:
                    if x[2] !=  faculty:
                       count+=1
...
tim = [[("COMPSCI", "210"), ('COMPSCI', '225'), ("COMPSCI", "230"), ("COMPSCI", "220")],
       [("COMPSCI", "110"), ('COMPSCI', '120'), ("ACCTG", "151G")],
       [("CAREER", "100G"), ('COMPSCI', '340'), ("COMPSCI", "250")],
       [("PHIL", "105"), ('BIOSCI', '101'), ("COMPSCI", "130"), ("COMPSCI", "351"), ("COMPSCI", "315")]]
tim = [[('CHEM', '110'), ('CHEM', '120')], [('CHEM', '251'), ('CHEM', '252'), ('CHEM', '253'), ('CHEM', '351')]]
print("Not taken maths", a.reccomended_action("chemistry", tim))

tim = [[('CHEM', '110'), ('CHEM', '120')],
       [('CHEM', '251'), ('CHEM', '252'), ('CHEM', '253'), ('CHEM', '351'), ("MATHS", ("108"))]]
print("Not taken maths", a.reccomended_action("chemistry", tim))

tim = [[('CHEM', '110'), ('CHEM', '120')],
       [('CHEM', '251'), ('CHEM', '252'), ('CHEM', '253'), ('CHEM', '351'), ("MATHS", "108"), ("CHEM", "330")]]
print("Not taken maths", a.reccomended_action("chemistry", tim))

tim = [[('CHEM', '110'), ('CHEM', '120')], [('CHEM', '260'), ("ACCTG", "151G"), ("BIOSCI", "100G")],
       [('CHEM', '251'), ('CHEM', '252'), ('CHEM', '253'), ('CHEM', '351'), ("MATHS", "108"),
